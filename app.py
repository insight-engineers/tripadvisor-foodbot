...
from utils.tools import get_restaurant_recommendations_tool
...
async def start():
    ...
    agent = ReActAgent.from_tools([get_restaurant_recommendations_tool], llm=llm_model, verbose=True)
    cl.user_session.set("agent", agent)  # Store the agent in session

    await cl.Message(
        author="Assistant", content="Hello, sir! I'm here to help you with your restaurant queries."
    ).send()
...
async def main(message: cl.Message):
    ...
    await cl.Message(author="Assistant", content=str(response)).send()
